...
public class SetLockActivity extends BaseActivity {
    ...
    private void patternComplete(List<LockPatternView.Cell> cells) {
        if (cells.size() < 4) {
            lockView.setPattern(LockPatternView.DisplayMode.DEFAULT);
            tvMessage.setText(WonderfulToastUtils.getString(R.string.gesturesPassThirdPartTip6));
        }
        if (type == 0) {//设置
            if (SetLockActivity.this.cells == null && cells.size() >= 4) {
                SetLockActivity.this.cells = new ArrayList<>(cells);
                tvMessage.setText(WonderfulToastUtils.getString(R.string.gesturesPassThirdPartTip7));
                lockView.setPattern(LockPatternView.DisplayMode.DEFAULT);
        ...
        if (SetLockActivity.this.cells != null) {
            if (isSame(SetLockActivity.this.cells, cells)) {
                if (type == 0) {
                    tvMessage.setText(WonderfulToastUtils.getString(R.string.set_up_success));
                    saveCells(cells);
                    lockView.setPattern(LockPatternView.DisplayMode.DEFAULT);
                } else if (type == 1) {
                    tvMessage.setText(WonderfulToastUtils.getString(R.string.Abolish_success));
                    clearCells();
                }
                finish();
                ...
                if (type == 0) {
                    tvMessage.setText(WonderfulToastUtils.getString(R.string.pwd_diff));
                } else if (type == 1) {
                    tvMessage.setText(WonderfulToastUtils.getString(R.string.Password_error));
                }
            }
        }
    }
