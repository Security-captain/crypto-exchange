...
import static cn.bizzan.utils.okhttp.WonderfulOkhttpUtils.post;
...
//                        dataCallback.onDataNotAvailable(GlobalConstant.TOKEN_DISABLE1, object.optString("message"));
    ...
    public void banners(String sysAdvertiseLocation, final DataCallback dataCallback) {
        WonderfulOkhttpUtils.post().url(UrlFactory.getBannersUrl()).addParams("sysAdvertiseLocation", sysAdvertiseLocation).addParams("lang", "CN").build().execute(new StringCallback() {
            @Override
            public void onError(Request request, Exception e) {
                super.onError(request,e);
