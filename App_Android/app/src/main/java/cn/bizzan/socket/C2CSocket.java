...
public class C2CSocket implements ISocket {
    ...
    public static synchronized C2CSocket getInstance() {
        return c2csocket == null ? c2csocket = new C2CSocket() : c2csocket;
    }
    ...
    @Override
    public void sendRequest(final CMD cmd, final byte[] body, ISocket.TCPCallback callback) {
        if (socketThread == null || !socketThread.isAlive()) {
            socketThread = new SocketThread();
            socketThread.start();
    ...
    public static byte[] buildRequest(CMD cmd, byte[] body) {
        ...
        try {
            ...
            WonderfulLogUtils.logi("请求的头部信息", "cmd:" + cmd.getCode() + ",length:" + length + ",sequenceId:" +
                    sequenceId + new String(body));
    ...
    public void dealResponse(final TCPCallback tcpCallback) throws IOException {
        ...
        WonderfulLogUtils.logd("返回的固定信息", "cmd:" + code + ",length:" + length + ",sequenceId:" +
