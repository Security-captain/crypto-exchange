...
public class KChartView extends BaseKChartView {
    ...
    private void initView() {
        ...
        addChildChidDraw("MACD", mMACDDraw);
        addChildChidDraw("KDJ", mKDJDraw);
        addChildChidDraw("RSI", mRSIDraw);
...
//                }
     ...
     * @param isBOLL
     */
    public void setMAandBOLL(boolean isMA, boolean isBOLL) {
        mMainDraw.showViewByMAandBOLL(isMA, isBOLL);
     ...
     * 设置副图是否显示
     */
    public void setChidType(int chidType) {
        setChildChildDraw(chidType);
        if (chidType >= 0 && lastType >= 0 || lastType < 0 && chidType < 0) {
            notifyChanged();
        } else {
